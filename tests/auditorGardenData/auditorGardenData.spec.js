...
function readThaiProvinceSample() {
  ...
  const province = provinces.find((p) => Array.isArray(p.amphure) && p.amphure.length);
  const district = province?.amphure?.find((a) => Array.isArray(a.tambon) && a.tambon.length);
...
test.describe("บันทึกข้อมูลประจำสวนยาง - ผู้ตรวจประเมิน", () => {
  ...
  test("TC-006: เปลี่ยนจำนวนรายการต่อหน้า", async ({ page }) => {
    ...
    if (await nativeSelect.count()) {
      ...
      await clickAndWaitInspectionsReload(page, async () => {
        await page.locator(".p-dropdown-items .p-dropdown-item").getByText("25", { exact: true }).click();
      });
    }
  ...
  test("TC-014: สลับแท็บ “อยู่ระหว่างการตรวจประเมิน”", async ({ page }) => {
    await clickAndWaitInspectionsReload(page, () =>
      page.getByRole("button", { name: TAB_IN_PROGRESS, exact: true }).click()
    );
...
test.describe("บันทึกข้อมูลประจำสวนยาง - Step 2", () => {
  ...
  test("TC-020: กรอกข้อมูลรายการปลูก 1 แถว", async ({ page }) => {
    await chooseAutoCompleteByTypingExact(page, "เลือกพันธุ์ยางพารา", "RRIT 251");
    ...
    await expect(page.getByPlaceholder("เลือกพันธุ์ยางพารา").first()).toHaveValue(
      "RRIT 251"
    );
    await expect(page.getByPlaceholder("ระยะปลูก (เมตร)").first()).toHaveValue("3");
    await expect(page.getByPlaceholder("จำนวนต้น").first()).toHaveValue("100");
    await expect(page.getByPlaceholder("เลือกวันที่ปลูก").first()).not.toHaveValue("");
  });
  ...
  test("TC-021: ระบบการให้น้ำ: เลือก “ไม่มี” → ช่อง “ระบุ” disabled", async ({
    ...
    const textarea = page.getByPlaceholder("ระบุรายละเอียดระบบการให้น้ำ").first();
  ...
  test("TC-023: ปุ๋ยเคมี: เพิ่ม/ลบรายการ", async ({ page }) => {
    const chemicalSection = page.locator("h5", { hasText: "ปุ๋ยเคมี" }).locator("..");
    ...
    const removeButton = chemicalSection.getByRole("button", { name: "ลบ" }).first();
  ...
  test("TC-024: ปุ๋ยเคมี: กรอกข้อมูลรายการ", async ({ page }) => {
    ...
    await chemicalSection.getByPlaceholder("สูตร/ชื่อปุ๋ย").first().fill("ยูเรีย");
    ...
    await expect(chemicalSection.getByPlaceholder("สูตร/ชื่อปุ๋ย").first()).toHaveValue(
      "ยูเรีย"
    );
    await expect(
      chemicalSection.getByPlaceholder("เช่น 2 กก./ไร่ หรือ 200 ก./ต้น").first()
    ).toHaveValue("2 กก./ไร่");
    await expect(chemicalSection.getByPlaceholder("ครั้ง/ปี").first()).toHaveValue("1");
  });
  ...
  test("TC-025: ปุ๋ยอินทรีย์: เพิ่ม/ลบรายการ", async ({ page }) => {
    const organicSection = page
      ...
      .locator("..");
    const formulaInputs = organicSection.getByPlaceholder("เช่น ปุ๋ยคอก, น้ำหมัก");
    await expect(formulaInputs).toHaveCount(1);

    await page.getByRole("button", { name: "เพิ่มปุ๋ยอินทรีย์" }).click();
    await expect(formulaInputs).toHaveCount(2);

    const removeButton = organicSection.getByRole("button", { name: "ลบ" }).first();
    await removeButton.click();
    await expect(formulaInputs).toHaveCount(1);
  });

  test("TC-026: ปุ๋ยอื่นๆ: เพิ่ม/ลบรายการ", async ({ page }) => {
    const otherSection = page.locator("h5", { hasText: "อื่นๆ" }).locator("..");
    ...
    const removeButton = otherSection.getByRole("button", { name: "ลบ" }).first();
    await removeButton.click();
    await expect(formulaInputs).toHaveCount(1);
  });
  ...
  test("TC-027: ประวัติการใช้พื้นที่: เลือก “ใช้ประโยชน์” → ช่องปี enable", async ({
    ...
    const section = page.locator("div", { hasText: "4. ประวัติการใช้พื้นที่การผลิต" }).first();
  ...
  test("TC-030: ศัตรูพืช/โรค: กรอกข้อมูลรายการ", async ({ page }) => {
    await page.getByPlaceholder("ชื่อศัตรูพืช/โรค/อาการ").first().fill("เพลี้ย");
    await page.getByPlaceholder("เช่น ฤดูฝน, ฤดูแล้ง").first().fill("ฤดูฝน");
    await page.getByPlaceholder("ระบุวิธีการป้องกันและการจัดการ").first().fill("ตัดแต่งกิ่ง");

    await expect(page.getByPlaceholder("ชื่อศัตรูพืช/โรค/อาการ").first()).toHaveValue("เพลี้ย");
    await expect(page.getByPlaceholder("เช่น ฤดูฝน, ฤดูแล้ง").first()).toHaveValue("ฤดูฝน");
    await expect(page.getByPlaceholder("ระบุวิธีการป้องกันและการจัดการ").first()).toHaveValue("ตัดแต่งกิ่ง");
  });
  ...
  test("TC-031: พืชข้างเคียง: เลือก “ไม่มี” → ปิดการกรอกและปุ่มเพิ่ม", async ({
    ...
    const plantInput = page.getByPlaceholder("ระบุชนิดพืชที่ปลูกข้างเคียง").first();
