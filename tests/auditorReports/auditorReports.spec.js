...
async function mockInspectionSummaryApis(page, { inspection, items, farm }) {
  ...
  await page.route("**/api/v1/inspection-items**", async (route) => {
    ...
    if (url.searchParams.get("inspectionId") === String(inspection.inspectionId))
...
test.describe("สรุปผลการตรวจประเมินสวนยางพารา - ผู้ตรวจประเมิน", () => {
  ...
  test("TC-002: แสดงหัวข้อ/คำอธิบาย และปุ่มแท็บ", async ({ page }) => {
    ...
    await mockReportsList(page, () => ({ results: [], paginator: { total: 0 } }));
    ...
    await expect(
      page.getByText("จัดการและสรุปผลการตรวจประเมินสวนยางพารา", { exact: true })
    ).toBeVisible();
    await expect(page.getByRole("button", { name: "รอสรุปผล" })).toBeVisible();
  ...
  test("TC-006: กด “ล้างค่า” รีเซ็ตตัวกรอง", async ({ page }) => {
    ...
    const defaultRows = [
      makeInspection({
        ...
        rubberFarm: makeRubberFarm({ province: "สงขลา", district: "หาดใหญ่" }),
      }),
    ];
    const filteredRows = [
      makeInspection({
    ...
    await mockReportsList(page, (url) => {
      ...
      return {
        ...
        paginator: { total: isFiltered ? filteredRows.length : defaultRows.length },
      };
    });
  ...
  test("TC-007: แสดงคอลัมน์หลักของตาราง", async ({ page }) => {
    ...
    await mockReportsList(page, () => ({ results: rows, paginator: { total: 1 } }));

    await page.goto("/auditor/reports", { waitUntil: "domcontentloaded" });

    await expect(page.getByRole("columnheader", { name: "รหัสการตรวจ" })).toBeVisible();
    await expect(page.getByRole("columnheader", { name: "เกษตรกร" })).toBeVisible();
    await expect(page.getByRole("columnheader", { name: "สถานที่" })).toBeVisible();
    await expect(page.getByRole("columnheader", { name: "วันที่ตรวจ" })).toBeVisible();
  ...
  test("TC-011: แสดงหัวข้อ/คำอธิบาย และหัวข้อ section หลัก", async ({
    ...
    await expect(page.getByRole("heading", { name: "ข้อมูลทั่วไป" })).toBeVisible();
    await expect(
    ...
    await expect(summaryHeadings.nth(1)).toBeVisible();
  });
  ...
  test("TC-012: ตาราง “ผลการตรวจประเมินรายหัวข้อ” และนำทางไปหน้า detail", async ({
    ...
    await expect(page.getByRole("columnheader", { name: "ลำดับ" })).toBeVisible();
    await expect(
  ...
  test("TC-015: กรณี pending: บันทึกไม่สำเร็จ", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "บันทึกผลการประเมิน" }).click();
    await expect(page.getByText("บันทึกไม่สำเร็จ (mock)", { exact: true })).toBeVisible();
  });
  ...
  test("TC-017: กรณี pending: error ระดับ network/exception", async ({
    ...
    await page.getByRole("button", { name: "บันทึกผลการประเมิน" }).click();
    await expect(
      page.getByText("เกิดข้อผิดพลาดในการบันทึกผลการประเมิน", { exact: true })
    ).toBeVisible();
  });
  });

  test.describe(
    "หน้ารายละเอียดรายการตรวจ (/auditor/inspection-detail/:id/:itemId)",
    () => {
  ...
  test("TC-019: แสดงหัวข้อและข้อมูลสรุปการตรวจ", async ({ page }) => {
    ...
    await expect(page.getByText("รหัสการตรวจประเมิน", { exact: true })).toBeVisible();
    await expect(page.getByText("วันที่นัดหมาย", { exact: true })).toBeVisible();
    await expect(page.getByText("ประเภทการตรวจ", { exact: true })).toBeVisible();
    await expect(page.getByText("พื้นที่สวนยาง", { exact: true })).toBeVisible();
  });
  ...
  test("TC-020: แสดงหัวข้อรายการ + ส่วนข้อกำหนด (หรือ empty)", async ({
    ...
    await expect(
      page.getByText("รายการที่ 1 : น้ำ", { exact: true })
    ).toBeVisible();
    await expect(page.getByText("ข้อกำหนด", { exact: true })).toBeVisible();
  ...
  test("TC-021: แสดง “ข้อมูลเพิ่มเติม” ตามประเภท item (ตัวอย่าง: น้ำ)", async ({
    ...
    await expect(page.getByText("ข้อมูลเพิ่มเติม", { exact: true })).toBeVisible();
    await expect(
