...
export function useFarmerCertificates(initialRows = 10) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("Failed to fetch certificates for farmer");
      const data = await resp.json();
      setItems(data.results || []);
      setTotalRecords(data.paginator?.total ?? (data.results || []).length);
    } catch (err) {
      console.error("useFarmerCertificates fetch error:", err);
      setItems([]);
      setTotalRecords(0);
    } finally {
      setLoading(false);
