...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const summary = await adminReportService.getCertificateReportSummary(startDate, endDate);
    return NextResponse.json(summary);
  } catch (error) {
    console.error("Error fetching certificate report summary:", error);
    return NextResponse.json(
      { message: "Failed to fetch certificate report summary" },
      { status: 500 }
    );
  }
}
