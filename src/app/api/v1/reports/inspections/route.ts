...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const summary = await adminReportService.getInspectionReportSummary(startDate, endDate);
    return NextResponse.json(summary);
  } catch (error) {
    console.error("Error fetching inspection report summary:", error);
    return NextResponse.json(
      { message: "Failed to fetch inspection report summary" },
      { status: 500 }
    );
  }
}
