...
export class PlantingDetailController extends BaseController<PlantingDetailModel> {
  ...
  async createPlantingDetail(req: NextRequest): Promise<NextResponse> {
    try {
      ...
      const yearOfTapping = data.yearOfTapping
        ? new Date(data.yearOfTapping)
        : new Date();
      const monthOfTapping = data.monthOfTapping
        ? new Date(data.monthOfTapping)
        : new Date();

      const plantingDetail =
        await this.plantingDetailService.createPlantingDetail(
          {
            ...data,
  ...
  async getPlantingDetailsByRubberFarmId(
    ...
    try {
      ...
      const details =
        await this.plantingDetailService.getPlantingDetailsByRubberFarmId(
          rubberFarmId
        );
  ...
  async updatePlantingDetail(
    ...
    try {
      ...
      const updatedDetail =
        await this.plantingDetailService.updatePlantingDetail(
          detailId,
          data,
          userId
        );
  ...
  async delete(
    ...
    try {
      ...
      const isDeleted = await this.plantingDetailService.delete(
        detailId,
        userId
      );
      ...
      if (!isDeleted) {
        return NextResponse.json(
          { message: "Planting detail not found or could not be deleted" },
          { status: 404 }
        );
      }

      return NextResponse.json(
        { message: "Planting detail deleted successfully" },
        { status: 200 }
      );
    } catch (error) {
      return this.handleControllerError(error);
